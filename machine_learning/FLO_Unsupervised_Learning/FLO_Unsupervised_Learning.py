...
flodf = pd.read_csv(r".6-machine_learning/FLO_Unsupervised_Learning/flo_data_20k.csv")
df = flodf.copy()
check_df(flodf)
...
df["recency_years"] = (analysis_date - df["last_order_date"]).dt.total_seconds() / (365.25 * 24 * 60 * 60)
...
# Normal dağılım sağlamayanlar için;
...
#Standartlastırılmalarına ihtiyaç var mı diye. Peki neden standartlaştırıyoruz??
## verı setınde yer alan müşterileri  12  değişkene göre cluster etmemız(kumelere ayırmamız) gerekıyor.
## neden standartlastırma yapıyoruz??
## 1.degıskenlerın bırbırlerını ezmemesı ıcın
## 2.gradıent descent temellı optımızasyon yontemlerı kullanıldıgında, optımızasyon suresıne pozıtıf katkı saglayacagı ıcın
## 3.uzaklık temellı yontemler ( benzerlık/benzemezlık temellı yontemler ) soz konusu oldugunda bu degıskenlerın bırbırını ezmesı
## durumu gundem olacagından degıskenlerın standartlastırılması gerekmektedır demıstık.
...
#küme sayısı belirledik 7
#belirlediğimiz küme sayısı ile model oluşturuyoruz.
